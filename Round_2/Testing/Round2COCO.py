...
class Trader:
    ...
    def get_estimated_fair_price(self, symbol):
...
###############################################################
        ...
        mask_symbol = self.df_data_market["symbol"] == symbol
        df_symbol = self.df_data_market[mask_symbol]
        # We now get the mean of the past 50 vwap prices
        if len(df_symbol) >= 10:
            A = df_symbol.tail(5)
        else:
            A = df_symbol
        ...
        predicted_price = old_price
        print(str(predicted_price))
        return predicted_price
    
    def get_prev_fair_price(self, symbol):
...
###############################################################
    ...
    def get_orders_both_sides_quoted(self, symbol, state, current_pos, position_limit):
        ...
        market_spread1 = market_values["average_spread"]
        ...
        # else:
        fair_value_asset = market_values["average_value"]
        if market_spread1 < buy_spread:
            fair_value_asset = self.get_alternate_buy_price_asset(symbol, state)
        if market_spread1 < sell_spread:
            fair_value_asset = self.get_alternate_sell_price_asset(symbol, state)
        
        prev_price = self.get_prev_fair_price(symbol)
        ...
        if symbol == "COCONUTS" and state.timestamp > 1000:
                        
            #get PC data
            mask_symbol = self.df_data_market['symbol'] == "PINA_COLADAS"
            df_symbol = self.df_data_market[mask_symbol]

            df_stamped1 = df_symbol.loc[df_symbol['timestamp'] == (state.timestamp-100)]
            df_stamped2 = df_symbol.loc[df_symbol['timestamp'] == (state.timestamp-200)]
            last_PC_price2 = df_stamped2["vwap_price"]
            last_PC_price2 =last_PC_price2.values[0]
            last_PC_price1 = df_stamped1["vwap_price"]
            last_PC_price1 =last_PC_price1.values[0]

            # current_PC_values = self.get_fair_price_asset("PINA_COLADAS",state)   
            # current_PC_price = current_PC_values["average_value"]
            if last_PC_price1 > last_PC_price2:
                market_values = self.get_fair_price_asset(symbol, state)
                fair_value_asset = market_values["average_value"] + 1       

            elif last_PC_price1 > last_PC_price2:
                market_values = self.get_fair_price_asset(symbol, state)
                fair_value_asset = market_values["average_value"] + 1      
        ...
        # if prev_price > fair_value_asset and (-current_pos)/position_limit < 0.75:
        ...
        if (-current_pos)/position_limit < 0.75:
                ...
                sell_volume = - mt.floor((position_limit + current_pos)/4)
                orders.append(Order(symbol, sell_price, sell_volume))
                print("Trying to SELL " + str(symbol) + " price: ", str(sell_price) + " volume: ", str(sell_volume))            # sell_price = mt.ceil(fair_value_asset) + 2
        ...
        # if estimated_price < fair_value_asset and total_volume_buy == 0 and (current_pos)/position_limit < 0.75:

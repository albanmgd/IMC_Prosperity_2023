    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for symbol in state.listings.keys():
            ...
            if symbol == "PINA_COLADAS" or symbol == "COCONUTS":
                ...
                #             + sum([abs(k) * abs(v) for k, v in order_depth.sell_orders.items()])) \
                #             / (sum(order_depth.buy_orders.values()) - sum(order_depth.sell_orders.values()))
                # fair_value_asset = vwap
                # Compute the fair price based on the EMA
                prices = list(order_depth.buy_orders.keys()) + list(order_depth.sell_orders.keys())
                fair_value_asset = self.compute_ema(prices, alpha=0.5)
                ...
                # add penalizing factor to spread if we're already long
                fair_buy_price = fair_value_asset - buy_spread / 2
                # if current_pos > 0:
                #     fair_buy_price = fair_value_asset - buy_spread / (2 + current_pos)
                # else: fair_buy_price = fair_value_asset - buy_spread / 2  # Willing to buy lower than my valuation
                ...
                if (len(order_depth.buy_orders) != 0):
                    total_volume_sell = sum(value for key, value in order_depth.buy_orders.items() if key > fair_sell_price)

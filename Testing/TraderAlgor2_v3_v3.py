...
class Trader:
    ...
    # Initialize the method output dict as an empty dict
        ...
        for symbol in state.listings.keys():
            # Initialize the list of orders and profit for this symbol
            orders: list[Order] = []
            profit = 0

            # Retrieve the order depth for the symbol
            order_depth: OrderDepth = state.order_depths[symbol]

            # Get the current position on the symbol
            current_pos = state.position.get(symbol, 0)

            # Check if both sides of the market are quoted
            if (len(order_depth.sell_orders) > 0) and (len(order_depth.buy_orders) > 0):
                # Get the orders to send based on the strategy
                orders = self.get_orders_both_sides_quoted(symbol, state, current_pos, position_limit)

                # Calculate the profit based on the orders and the current position
                for order in orders:
                    if order.side == "BUY":
                        profit -= order.price * order.volume
                    else:
                        profit += order.price * order.volume
                    current_pos += order.volume

            # Trim the orders to fit the position limit and add them to the result dictionary
            orders = self.trim_orders(symbol, state, orders)
            result[symbol] = {"orders": orders, "profit": profit}

...
class Trader:
    ...
    limits = {"PEARLS": 20, "BANANAS": 20, "COCONUTS": 600, "PINA_COLADAS": 300,
              "BERRIES": 250, "DIVING_GEAR": 50, "PICNIC_BASKET": 70}
        ...
        for symbol in state.listings.keys():
            ...
            if (len(order_depth.sell_orders) > 0) and (len(order_depth.buy_orders) > 0):
                ...
                if symbol == "PICNIC_BASKET":
                    orders = self.get_orders_basket(state)
                    orders = self.trim_orders(symbol, state, orders)
                else:
                    continue
    ...
    def trim_orders(self, symbol, state, orders):
        ...
        if buys:
            ...
            if symbol == "PEARLS" or symbol == "BANANAS":
                ...
                if buy_surplus < 0:
                    print("We have room to grow on buy side", symbol, "Position is", position, "we want to buy",
                          sum_buy)
        ...
        if sells:
            ...
            if symbol == "PEARLS" or symbol == "BANANAS":
                if sell_surplus < 0:
                    print("We have room to grow on sell side", symbol, "Position is", position, "we want to sell",
                          sum_sell)
    ...
    def get_mid_price(symbol, state):
        ...
        return {"mid_price": mid_price, "best_bid": best_bid, "best_ask_vol": best_ask_vol, "best_ask": best_ask,
    ...
    def get_orders_coco_pina(self, symbol, state, current_pos, position_limit):
        ...
        if symbol == "COCONUTS":
            ...
            if (PC_value / COCO_value) > upper_limit and old_ratio_min == ratio:
                ...
                sell_volume = -min(best_COCO_bid_vol,
                                   -round(best_PC_ask_vol * fair_limit))  # making sure the position is balanced
                orders.append(Order(symbol, sell_price, sell_volume))
        ...
        if symbol == "PINA_COLADAS":
            ...
            if PC_value / COCO_value > upper_limit and old_ratio_min == ratio:
                ...
                sell_volume = -min(best_PC_bid_vol,
                                   round(-best_COCO_ask_vol / fair_limit))  # * (1.87294-PC_value/COCO_value)/0.003696
                orders.append(Order(symbol, sell_price, sell_volume))
                print("SELL " + str(symbol) + " price: ", str(sell_price) + " volume: ", str(sell_volume))
    ...
    def get_orders_diving_gear(self, symbol, state, current_pos, position_limit):
        ...
        if current_pos > 0:
            if GEAR_value < current_ratio and (state.timestamp - self.jump_timestamps[-1]) > 400:
    ...
    def get_orders_berries(self, state: TradingState):
        ...
        if 5000 < state.timestamp < 350000:
            ...
            if own_trades is not None:
                mask_timestamp = state.timestamp - own_trades[
                    0].timestamp > timestamp_delta  # Adding the [0] in case we did multiple; not supposed to happen
    ...
    def get_orders_basket(self, state, symbol="PICNIC_BASKET"):
        # going to put orders based on last+im+v HFT method
        # parameters we're gonna need: Pt=> price of the most recent tstamp
        # Pt_1 => price of the second most recent tstamp
        # Ts => tick size, to be determined manually by experiment
        # lambda1 => evolution rate also manually
        # lambda2 => uptrend/downtrend increment value also manually
        # Mu_H => absorption rate also manually
        # P_flu => price fluctuation, calculated
        # HF_at => our action threshold manually // we can bypass this threshold, and start after having 2 trade prices
        # now let's get to work
        order_depth = state.order_depths[symbol]
        pt = (min(order_depth.sell_orders.keys()) + max(order_depth.buy_orders.keys())) / 2
        if len(self.picnic_pt) <= 51:
            self.picnic_pt.append(pt)  # adding the most recent
        if len(self.picnic_pt) > 51:
            self.picnic_pt.pop(0)   # removing the non useful data

        orders = []
        last5 = np.mean(self.picnic_pt[-15:])
        last30 = np.mean(self.picnic_pt)
        previous_last5 = np.mean(self.picnic_pt[-16:-1])
        previous_last30 = np.mean(self.picnic_pt[:-1])
        if last30 - last5 >= 100:
            self.picnic_trend["down"] = True
        if previous_last30 > previous_last5 and last30 < last5:
            # if we've been in downtrend, and we aren't anymore
            self.picnic_trend["down"] = False

        if last5 - last30 >= 100:
            self.picnic_trend["up"] = True
        if previous_last30 < previous_last5 and last30 > last5:
            self.picnic_trend["up"] = False
        ...
        if len(self.picnic_pt) == 51:
            ...
            vol_buys = sum(order_depth.buy_orders.values())
            vol_sells = -sum(order_depth.sell_orders.values())
            volume_diff = (vol_buys - vol_sells) / (vol_buys + vol_sells)

            pt_1 = self.picnic_pt[-2]
            pt = self.picnic_pt[-1]
            order_volume = int((vol_buys + vol_sells) * 0.5 * Mu_H)

            if volume_diff > lambda1:
                sell_price = pt + (abs(pt - pt_1) + lambda2) * tick_size
                buy_price = pt - abs(pt-pt_1) * tick_size
            elif -volume_diff > lambda1:
                sell_price = pt + abs(pt - pt_1) * tick_size
                buy_price = pt - (abs(pt - pt_1) + lambda2) * tick_size
            else:
                sell_price = pt + (abs(pt - pt_1) + lambda2/2) * tick_size
                buy_price = pt - (abs(pt - pt_1) + lambda2/2) * tick_size
            if not self.picnic_trend["down"]:
                if self.picnic_trend["up"]:
                    order_volume += int((self.limits[symbol] - self.get_pos_symbol(symbol, state)) / 2)
                orders.append(Order(symbol, buy_price, order_volume))
            if not self.picnic_trend["up"]:
                if self.picnic_trend["down"]:
                    order_volume += int((self.limits[symbol] + self.get_pos_symbol(symbol, state)) / 2)
                orders.append(Order(symbol, sell_price, -order_volume))

...
class Trader:
    ...
    def trim_orders(self, symbol, state, orders):
        ...
        if buys:
            ...
            if symbol != "COCONUTS" and symbol != "PINA_COLADAS" and symbol !="DIVING_GEAR":
    ...
    def get_orders_both_sides_quoted(self, symbol, state, current_pos, position_limit):
        ...
        # ratios_mav5 = gear_price_series.rolling(window=20, center = False).mean()
        ...
        if current_pos > 0:
            if GEAR_value < last_25_avg and (state.timestamp - self.jump_timestamps[-1])> 5000:

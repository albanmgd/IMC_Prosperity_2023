...
class Trader:
    ...
    def get_orders_both_sides_quoted(self, symbol, state, current_pos, position_limit):
        ...
        # Now we will make sure to cut the orders volume to properly fit the set limits
        # getting the product position
        buy_position: int = current_pos
        sell_position: int = current_pos

        total_volume_buy = 0
        buy_orders_to_place = []
        # Check if there are any SELL orders in the  market and if we can issue a buy order
        if len(order_depth.sell_orders) != 0 and buy_position < position_limit:
            # Adding volume until we reach a price where we don't to buy anymore
            # or until we can't buy anymore, i.e. position = limit

            for key, value in order_depth.sell_orders.items():
                if key < fair_buy_price and buy_position < position_limit:
                    # if the volume makes us exceed the limit, all orders will be cancelled
                    # making sure this is not going to happen
                    if buy_position - value > position_limit:
                        value = buy_position - position_limit
                    # adding the order without the risk of getting over the limit
                    total_volume_buy += value
                    buy_orders_to_place.append((key, value))
                    buy_position -= value  # coming from a sell order, quantity is negative
        ...
        if len(order_depth.buy_orders) != 0 and sell_position > -position_limit:

            for key, value in order_depth.buy_orders.items():
                if key > fair_sell_price and sell_position > -position_limit:
                    # if the volume makes us exceed the limit, all orders will be cancelled
                    # making sure this in not going to happen
                    if sell_position - value < -position_limit:
                        value = -position_limit - sell_position
                    # adding the order without the risk of getting over the limit
                    total_volume_sell += value
                    sell_orders_to_place.append((key, value))
                    sell_position -= value

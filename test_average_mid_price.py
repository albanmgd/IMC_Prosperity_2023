...
class Trader:
    ...
    pearls_averages = []
    pearls_tstamps = 27
    # 10 avg => 2757 SHELLS// 20 avg => 2775 SHELLS// 30 avg => 2773 SHELLS// 40 avg => 2749SHELLS// 100 avg 2735 SHELLS
    # 25 avg => 2775 // 23 avg => 2769// 27 avg => 2793 SHELLS
    ...
    def get_fair_price_asset(self, symbol, state):
        ...
        if symbol == "PEARLS":
            # Take the current mid_price and add to the list
            current_mid = (min(order_depth.sell_orders.keys()) + max(order_depth.buy_orders.keys()))/2
            self.pearls_averages.append(current_mid)

            # check if we had enough before, now we have number_tstamps+1 mid_prices, so we pop first one
            if len(self.pearls_averages) > self.pearls_tstamps:
                self.pearls_averages.pop(0)

            # now if we have our number_tstamp mid_prices, we define their average as the fair_price
            if len(self.pearls_averages) == self.pearls_tstamps:
                return (sum(self.pearls_averages)) / self.pearls_tstamps
    ...
    def get_orders_both_sides_quoted(self, symbol, state, current_pos, position_limit):
        ...
        # if we have calculated enough averages for PEARLS, we use the fair_value directly as buy/sell indicator
        if symbol == "PEARLS" and len(self.pearls_averages) == self.pearls_tstamps:
            fair_sell_price = fair_value_asset
            fair_buy_price = fair_value_asset
